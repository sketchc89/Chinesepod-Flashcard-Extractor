...
driver = webdriver.Chrome(executable_path= os.path.join(root_dir, '/usr/bin/chromedriver'),
                               chrome_options=chrome_profile)
driver.get("chrome://plugins/")

credentials = read_credentials(os.path.join(root_dir, 'login.txt'))
...
source_dir = temp_dir
target_dir = os.path.join(os.path.expanduser('~'), '.local/share/Anki2/User\ 1/')
